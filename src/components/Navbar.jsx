      ...
      <nav className={`navbar ${scrolled ? 'scrolled' : ''}`}>
        <div className="container" style={{ display:'flex', alignItems:'center', justifyContent:'space-between' }}>
                  ...
                  {active === link.href && (
                    <motion.span layoutId="nav-underline"
                      style={{ position:'absolute', bottom:'-3px', left:0,
                        height:'1px', width:'100%', background:'var(--emerald)', borderRadius:'1px' }} />
                  )}
                </a>
              </li>
            ))}
          ...
          <button onClick={() => setMenuOpen(!menuOpen)} className="burger-btn" aria-label="Menu"
            style={{ background:'none', border:'1px solid var(--border-mid)', borderRadius:'4px',
      ...
      <AnimatePresence>
        {menuOpen && (
          <motion.div key="mob"
            ...
            style={{ position:'fixed', inset:0, background:'var(--void)', zIndex:150,
            ...
            <div style={{ display:'flex', justifyContent:'space-between', alignItems:'center', marginBottom:'3rem' }}>
              ...
              <button onClick={() => setMenuOpen(false)}
                style={{ background:'none', border:'none', cursor:'pointer', color:'var(--white)' }}>
                <X size={24}/>
              </button>
            </div>
            ...
            <nav style={{ flex:1, display:'flex', flexDirection:'column', justifyContent:'center', gap:'0.25rem' }}>
              {links.map((link, i) => (
                <motion.a key={link.href} href={link.href}
                  ...
                  style={{ fontFamily:'var(--font-display)', fontSize:'clamp(2rem,7vw,2.8rem)', fontWeight:700,
                    color: active === link.href ? 'var(--emerald)' : 'var(--white)',
                    padding:'0.6rem 0', borderBottom:'1px solid var(--border)', transition:'color 0.2s' }}>
                  {link.label}
                </motion.a>
              ))}
            </nav>
