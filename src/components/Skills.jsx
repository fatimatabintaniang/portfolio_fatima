...
const catColors  = { frontend:'#5a9bd4', backend:'#2dd4a0', database:'#e8935a', devops:'#a07dc4', tools:'#d4c05a', design:'#d45a7d' };
...
export default function Skills() {
  ...
  return (
    <section id="skills" ref={ref} style={{ padding:'var(--section-pad) 0', background:'var(--deep)', position:'relative', overflow:'hidden' }}>
      <div aria-hidden style={{ position:'absolute', top:'50%', left:'-3%', transform:'translateY(-50%)', fontFamily:'var(--font-display)', fontSize:'clamp(180px,22vw,360px)', fontWeight:900, color:'transparent', WebkitTextStroke:'1px rgba(45,212,160,0.04)', lineHeight:1, userSelect:'none', pointerEvents:'none' }}>02</div>
      ...
      <div className="container" style={{ position:'relative', zIndex:1 }}>
        ...
        <div style={{ display:'flex', gap:'0.6rem', flexWrap:'wrap', marginBottom:'3rem' }}>
          {categories.map(cat => {
            ...
            return (
              <button key={cat} onClick={() => setActive(cat)} style={{ padding:'0.42rem 1.1rem', border:'1px solid '+(isA ? col : 'var(--border-dark)'), background: isA ? col+'18' : 'transparent', color: isA ? col : 'var(--mist)', fontFamily:'var(--font-mono)', fontSize:'0.7rem', letterSpacing:'0.1em', textTransform:'uppercase', cursor:'pointer', borderRadius:'100px', transition:'all 0.25s' }}>
                {catLabels[cat]}
              </button>
            );
          })}
        ...
        <div style={{ display:'grid', gridTemplateColumns:'repeat(auto-fill,minmax(265px,1fr))', gap:'1.25rem' }}>
          {filtered.map((s, i) => {
            ...
            return (
              <motion.div key={s.name} initial={{ opacity:0,y:18 }} animate={inView?{opacity:1,y:0}:{}} transition={{ delay:i*0.055, duration:0.5 }}
                style={{ background:'var(--surface)', border:'1px solid var(--border)', borderRadius:'6px', padding:'1.5rem 1.75rem', transition:'all 0.3s' }}
                onMouseEnter={e => { e.currentTarget.style.borderColor=color+'44'; e.currentTarget.style.boxShadow='0 8px 32px rgba(0,0,0,0.3),0 0 0 1px '+color+'22'; }}
                ...
                <div style={{ height:'3px', background:'rgba(255,255,255,0.06)', borderRadius:'4px', overflow:'hidden', marginBottom:'0.85rem' }}>
                  <motion.div initial={{ width:0 }} animate={inView?{ width:s.level+'%' }:{ width:0 }} transition={{ duration:1.1, delay:0.3+i*0.055, ease:[0.25,0.46,0.45,0.94] }} style={{ height:'100%', background:color, borderRadius:'4px' }} />
                </div>
